...
HeapRegion* G1CollectedHeap::new_heap_region(uint hrs_index,
                                             MemRegion mr) {
  return new (hrs_index)HeapRegion(hrs_index, bot(), mr);
}
...
G1CollectedHeap::G1CollectedHeap(G1CollectorPolicy* collector_policy) :
  ...
  //mhr: modify
  pair_array = NEW_C_HEAP_ARRAY(AddrPair, 524288, mtGC);
  _queue_bitmap = NEW_C_HEAP_ARRAY(size_t, 67108870, mtGC);
  memset(_queue_bitmap, 0 , 67108870*sizeof(size_t));
  gctime = 0;
  commtime = 0;
  regiontime = 0;
 ...
 *  
 */
G1RegionToSpaceMapper* G1CollectedHeap::create_aux_memory_mapper_at_fixed_addr(const char* description,
																																 char* request_addr,
																																 size_t size,
																																 size_t translation_factor) {
	size_t preferred_page_size = os::page_size_for_region_unaligned(size, 1);
	...
	log_debug(semeru,alloc)("%s, Reserve RegionToPage mapper for [0x%lx, 0x%lx) ",description, (size_t)request_addr, (size_t)(request_addr + size*HeapWordSize ) );
...
jint G1CollectedHeap::initialize() {
  ...
  if(SemeruEnableMemPool && UseCompressedOops==false){
    // Initialize the Semeru Heap

    size_t reserved_for_rdma_data = RDMA_STRUCTURE_SPACE_SIZE;	// Bytes, Reserved for structures transfered by RDMA.

    // max_byte_size is also controlled by -Xmx at CPU server now.
    heap_rs = Universe::reserve_semeru_memory_pool(max_byte_size + reserved_for_rdma_data,
                                                                heap_alignment);

    initialize_reserved_region((HeapWord*)heap_rs.base()+reserved_for_rdma_data/HeapWordSize, 
    ...
    // Carve out the reserved space for target object queue.
	  ...
	  rdma_rs = heap_rs.first_part(reserved_for_rdma_data);

    // Carve out the G1 part of the heap. The 2nd part of heap_rs is normal Java Heap.
    // Added by Chenxi.
  ...
  // ReservedSpace g1_rs = heap_rs.first_part(max_byte_size); // already reseved before 
  ...
  if(EnableBitmap) {
    ...
    prev_bitmap_storage = NULL;
    next_bitmap_storage = NULL;
  }
  ...
  //_bot = new G1BlockOffsetTable(reserved_region(), bot_storage);
  {
    char* start = (char*)(SEMERU_START_ADDR + BOT_GLOBAL_STRUCT_OFFSET);
		...
		_bot = new(size, start) G1BlockOffsetTable(reserved_region(), bot_storage);
...
int G1CollectedHeap::compare_meta_st(const AddrPair a, const AddrPair b) {
  size_t st_a = (size_t)a.st;
  size_t st_b = (size_t)b.st;
  if (st_a > st_b) {
    return 1;
  } else if (st_a == st_b) {
    return 0;
...
G1CollectedHeap::semeru_do_collection_pause_at_safepoint(double target_pause_time_ms) {
  ...
  //mhr: new
  //mhr: modify
  g1_policy()->collector_state()->set_in_young_only_phase(false);
  // We should not be doing initial mark unless the conc mark thread is running
  // if (!_cm_thread->should_terminate()) {
  //   // This call will decide whether this pause is an initial-mark
  //   // pause. If it is, in_initial_mark_gc() will return true
    ...
    { // Call to jvmpi::post_class_unload_events must occur outside of active GC
...

      {
        ...
        for(int mem_id=0; mem_id<NUM_OF_MEMORY_SERVER; mem_id++){
          if(recv_mem_server_cset()->num_of_enqueued_regions(mem_id)!=0){
            update_klass = true;
            break;
          }
        }
        ...
        if(update_klass){
          ...
          while(iter.repeat()){
            ClassLoaderMetaspace* clms = iter.get_next();
            if(clms != NULL)
              clms->send_metadata(pair_array, num_pair);
          }
          QuickSort::sort(pair_array, num_pair, G1CollectedHeap::compare_meta_st, true);

          double send_time_st = os::elapsedTime();
          ...
          for(int i = 0; i < num_pair; i ++){
            if(send_base == NULL) {
              ...
              send_end = (size_t)pair_array[i].ed;
              continue;
            }
            if((size_t)pair_array[i].st > send_end + 0x4000) {
              ...
              send_end = (size_t)pair_array[i].ed > send_end ?  (size_t)pair_array[i].ed : send_end;
            }
          ...
          double send_time_ed = os::elapsedTime();
          tty->print("Send MetaData: %lf\n", send_time_ed-send_time_st);
          commtime += send_time_ed-send_time_st;

        }
        
        //debug
        mem_cset_length = _collection_set._optional_region_length;
  ...
  //busy_wait_the_end_of_mem_server_compaction();

  // debug
  // added by chenxi
  // if(mem_cset_length != 0){
  //   log_debug(semeru,rdma)("%s, _collection_set._optional_region_length 0x%x , wait.\n", __func__, mem_cset_length);
  //   wait_here_for_debug();
  // }else{
  //   log_debug(semeru,rdma)("%s, _collection_set._optional_region_length 0x%x , Skip.\n", __func__, mem_cset_length);
  // }

  double all_time_ed = os::elapsedTime();
  tty->print("All gc: %lf\n", all_time_ed-read_time_st);
  gctime += all_time_ed-read_time_st;



  tty->print("Total gc and comm and region: %lf, %lf, %lf\n", gctime, commtime, regiontime);
  return true;
}
  ...
  void work(uint worker_id) {
...

    {
      ...
      size_t evac_term_attempts = 0;
      {
        ...
        double start_evac = os::elapsedTime();
...
class G1ParTaskNew : public AbstractGangTask {
protected:
  G1CollectedHeap*         _g1h;
  G1ParScanThreadStateSet* _pss;
  RefToScanQueueSet*       _queues;
...
public:
  G1ParTaskNew(G1CollectedHeap* g1h, G1ParScanThreadStateSet* per_thread_states, RefToScanQueueSet *task_queues, G1RootProcessor* root_processor, uint n_workers)
    : AbstractGangTask("G1 collection"),
      _g1h(g1h),
      _pss(per_thread_states),
      _queues(task_queues),
  ...
  void work(uint worker_id) {
...

    {
      ...
      //mhr: modify
      _g1h->g1_rem_set()->update_rem_set(pss, worker_id);
      // _g1h->g1_rem_set()->oops_into_collection_set_do(pss, worker_id);
...
//mhr: modify
class G1ParTaskAfter : public AbstractGangTask {
protected:
  G1CollectedHeap*         _g1h;
  G1ParScanThreadStateSet* _pss;
  RefToScanQueueSet*       _queues;
...
public:
  G1ParTaskAfter(G1CollectedHeap* g1h, G1ParScanThreadStateSet* per_thread_states, RefToScanQueueSet *task_queues, G1RootProcessor* root_processor, uint n_workers)
    : AbstractGangTask("G1 collection"),
      _g1h(g1h),
      _pss(per_thread_states),
      _queues(task_queues),
  ...
  void work(uint worker_id) {
...

    {
      ...
      // We only want to make sure that the oops in the nmethods are adjusted with regard to the
      // objects copied by the current evacuation.
      _g1h->g1_rem_set()->scan_rem_set(pss, worker_id);//use target obj queue
      ...
      //_g1h->g1_rem_set()->scan_rem_set_source(pss, worker_i);//use target obj queue
...
//mhr: modify
class G1ParTaskUpdate : public AbstractGangTask {
protected:
  G1CollectedHeap*         _g1h;
  G1ParScanThreadStateSet* _pss;
  RefToScanQueueSet*       _queues;
...
public:
  G1ParTaskUpdate(G1CollectedHeap* g1h, G1ParScanThreadStateSet* per_thread_states, RefToScanQueueSet *task_queues, G1RootProcessor* root_processor, uint n_workers)
    : AbstractGangTask("G1 collection"),
      _g1h(g1h),
      _pss(per_thread_states),
      _queues(task_queues),
  ...
  void work(uint worker_id) {
...

    {
      ...
      _g1h->g1_rem_set()->scan_rem_set_source(pss, worker_id);
    }
  }
};
...
//mhr: modify
class G1ParTaskMemUpdate : public AbstractGangTask {
protected:
  G1CollectedHeap*         _g1h;
  G1ParScanThreadStateSet* _pss;
  RefToScanQueueSet*       _queues;
...
public:
  G1ParTaskMemUpdate(G1CollectedHeap* g1h, G1ParScanThreadStateSet* per_thread_states, RefToScanQueueSet *task_queues, G1RootProcessor* root_processor, uint n_workers)
    : AbstractGangTask("G1 collection"),
      _g1h(g1h),
      _pss(per_thread_states),
      _queues(task_queues),
  ...
  void work(uint worker_id) {
...

    {
      ...
      G1ScanRSForRegionClosureMemUpdate cl(_g1h->g1_rem_set()->scan_state(), pss, worker_id);

      size_t len = _g1h->_mem_server_flags->_compacted_region_length;
      OrderAccess::loadload();
      if (len == 0) {
        return;
      }
      size_t start_pos = (worker_id * len) / _n_workers;
      size_t cur_pos = start_pos;
      do {
        HeapRegion* r = _g1h->region_at(_g1h->_mem_server_flags->_compacted_regions[cur_pos]);
...
void G1CollectedHeap::evacuate_collection_set(G1ParScanThreadStateSet* per_thread_states) {
...

  {
    ...
    _collection_set._collection_set_cur_length -= _collection_set._rebuild_set_length;
    ...
    size_t* server_1_cset = NEW_C_HEAP_ARRAY(size_t, max_regions()/2, mtGC);
    size_t server_0_len = 0;
    size_t server_1_len = 0;

    double send_region_info_st = os::elapsedTime();
    double send_region_tim = 0;

    for(size_t mem_id=0; mem_id< NUM_OF_MEMORY_SERVER; mem_id++){
      size_t num_mem_cset = *(_recv_mem_server_cset->num_received_regions(mem_id));
      for(size_t i = 0; i < num_mem_cset; i ++){
        uint hr_index = _recv_mem_server_cset->get(mem_id,i);
        HeapRegion* hr = region_at(hr_index);
        guarantee(hr != NULL, "Tried to access region %u that has a NULL HeapRegion*", hr_index);
        //hr->cross_region_ref_update_queue()->_marked_from_root = true;
        hr->cross_region_ref_target_queue()->_marked_from_root = true;
        hr->send_info_at_gc();
        hr->send_target_queue_at_gc();
        double send_region_st = os::elapsedTime();
        hr->flush_data();
        double send_region_ed = os::elapsedTime();
        send_region_tim += send_region_ed - send_region_st;
      } // end of i, each enqueed region
    ...
    double send_region_info_ed = os::elapsedTime();
    tty->print("Send region time all: %lf, only region time: %lf\n", send_region_info_ed - send_region_info_st, send_region_tim);
    commtime += send_region_info_ed - send_region_info_st;
    regiontime += send_region_tim;


    //send_evacuated_region_info();

    while (_collection_set._optional_region_length != 0) {
      // We want to return regions not evacuated to the
      // chooser in reverse order to maintain the old order.
      HeapRegion* hr = _collection_set.remove_last_optional_region();
      if(hr == NULL) {
        continue;
      }
      ...
      old_set_add(hr);
      clear_in_cset(hr);
      hr->set_index_in_opt_cset(G1OptionalCSet::InvalidCSetIndex);
    }
    _collection_set.free_optional_regions();



    for(size_t i = 0; i < _collection_set._collection_set_cur_length; i ++){
      HeapRegion* hr = region_at(_collection_set._collection_set_regions[i]);
      if(hr->is_old()) {
...
void G1CollectedHeap::close_stw_window(){
  


  // size_t num_mem_cset = *(_recv_mem_server_cset->num_received_regions());
  // //size_t num_mem_compacted_set = mem_server_flags()->_compacted_region_length;
  // for(size_t i = 0; i < num_mem_cset; i ++){
  //   size_t hr_index = _recv_mem_server_cset->get(i);
  //   HeapRegion* hr = region_at(hr_index);
  //   guarantee(hr != NULL, "Tried to access region %lu that has a NULL HeapRegion*", hr_index);
  //   hr->send_target_queue_at_gc();
  // }


  // Modify state  
  cpu_server_flags()->set_cpu_server_in_mutator();
    
  // inform Memory server
  send_cpu_server_flags_to_mem_server();

  log_debug(semeru,rdma)("%s, Close STW windown and inform all Memory Servers. \n", __func__);

  // cpu_server_flags()->_target_queue_sent = true;
  // send_cpu_server_flags_to_mem_server();


  // log_debug(semeru,rdma)("%s, Send complete target queue done. \n", __func__);
}

/**
 * Broadcast the evacated Region's information to other servers.
 * After Memory server receiving the data, it can start cross-region reference updating.
 */
void G1CollectedHeap::send_evacuated_region_info(){
  // Send the cross_region_ref_update_queue of evacuated Regions to related Memory Servers
  
  //
  // Wait to be written.
  //
  
  // Send CPU server to Memory server done.

  size_t len = _collection_set._collection_set_cur_length;
  OrderAccess::loadload();
  if (len == 0) {
    return;
  }
  for(size_t i = 0; i < len; i++) {
    HeapRegion* r = region_at(_collection_set._collection_set_regions[i]);
    r->send_target_queue_at_gc(); 
  }


  cpu_server_flags()->_cpu_server_data_sent = true;
  send_cpu_server_flags_to_mem_server();


  log_debug(semeru,rdma)("%s, Send CPU server data done, wait on the MS to stop current compacting. \n", __func__);
}

/**
 * Keep reading data until the mem_server_flags->mem_server_wait_on_exchange is ture.
 *  
 */
void G1CollectedHeap::read_data_from_memory_servers(){
  bool    mem_server_wait_on_exchange = false;
  size_t  regions_compacted  = 0;
  size_t  regions_data_received = 0;
  uint    region_index;
  HeapRegion* hr;

  log_debug(semeru,rdma)("%s, Read compacted Region infor from Memory Server. \n", __func__);

  // 1) Read the _mem_server_wait_on_data_exchange until it's true.
  // if _mem_server_wait_on_data_exchange true, means memory server stop claiming any Region and wait for data exchanging now.
  //
  while(mem_server_wait_on_exchange == false){
    read_mem_server_flags_from_mem_server();
    regions_compacted = mem_server_flags()->_compacted_region_length;
    //mem_server_wait_on_exchange = mem_server_flags()->_mem_server_wait_on_data_exchange;
    mem_server_wait_on_exchange = true;

    //2) Read the data incrementally
    while(regions_data_received < regions_compacted){
      region_index = mem_server_flags()->_compacted_regions[regions_data_received];
      hr = _hrm->at(region_index);
      hr->read_info_at_gc();

      regions_data_received++;
    }// end of data reading, while

  }

    
  log_debug(semeru,rdma)(" End of %s, compacted_region num 0x%lx, mem_wait_on_exchange ? %d, mem_on_compact ? %d ", 
                                                      __func__,
                                                      mem_server_flags()->_compacted_region_length, 
                                                      mem_server_flags()->_mem_server_wait_on_data_exchange, 
                                                      mem_server_flags()->_is_mem_server_in_compact);

}



void G1CollectedHeap::send_uncompacted_region_queue(){
  ...
  //   size_t hr_index = _recv_mem_server_cset->get(i);
  //   size_t j = 0;
  //   for(j = 0; j < num_mem_compacted_set; j++) {
  //     if((size_t)(mem_server_flags()->_compacted_regions[j]) == hr_index) {
  //       break;
  //     }
  //   }
  //   if(j == num_mem_compacted_set) {
  //     continue;
  //   }
  //   HeapRegion* hr = region_at(hr_index);
  //   guarantee(hr != NULL, "Tried to access region %lu that has a NULL HeapRegion*", hr_index);
...
/**
 * Wait the Memory server to finish its cross_region reference update.
 * 
 */
void G1CollectedHeap::busy_wait_the_end_of_mem_server_compaction(){
  
  log_debug(semeru,rdma)("%s, wait the end of cross_region update of Memory Servers", __func__);

  do{
    // check the _mem_server_wait_on_data_exchange agian.
      read_mem_server_flags_from_mem_server();
  }while(mem_server_flags()->_is_mem_server_in_compact);

  log_debug(semeru,rdma)("%s, CPU server are all done. Resume mutators ... ", __func__);
}


void G1CollectedHeap::wait_here_for_debug(){

    {    
      log_debug(semeru,mem_trace)("%s, Going to wait on SemeruCGC_lock. \n", __func__);
      bool wait_status;
      ...
      wait_status = SemeruCGC_lock->wait(Mutex::_no_safepoint_check_flag);
      log_debug(semeru,mem_trace)("%s,SemeruCGC_lock wait status %d. \n", __func__, wait_status);
...
//mhr: modify
class G1EvacuateOptionalRegionTaskNew : public AbstractGangTask {
  G1CollectedHeap* _g1h;
  G1ParScanThreadStateSet* _per_thread_states;
  G1OptionalCSet* _optional;
  RefToScanQueueSet* _queues;
  ...
  void scan_roots(G1ParScanThreadState* pss, uint worker_id) {
    ...
    for (uint i = _optional->current_index(); i < _optional->current_limit(); i++) {
      ...
      G1ScanRSForRegionClosureNew scan_rs_cl(_g1h->g1_rem_set()->scan_state(), &obj_cl, pss, G1GCPhaseTimes::OptScanRS, worker_id);
      scan_rs_cl.do_heap_region(hr);
      // copy_time += trim_ticks(pss);
      // scanned += scan_rs_cl.cards_scanned();
      // claimed += scan_rs_cl.cards_claimed();
  ...
  void scan_rem_set_source(G1ParScanThreadState* pss, uint worker_id) {
    // for (uint i = _optional->current_index(); i < _optional->current_limit(); i++) {
    //   HeapRegion* hr = _optional->region_at(i);
    //   // G1ScanRSForOptionalClosure scan_opt_cl(&obj_cl);
    //   // pss->oops_into_optional_region(hr)->oops_do(&scan_opt_cl, root_cls->raw_strong_oops());
    ...
    //   G1ScanObjsDuringScanRSClosureNew scan_cl(_g1h, pss);
    //   G1ScanRSForRegionClosureUpdate cl(_g1h->g1_rem_set()->scan_state(), &scan_cl, pss, G1GCPhaseTimes::ScanRS, worker_id);
    //   cl.do_heap_region(hr);
    ...
    // for (uint i = _optional->current_index(); i < _optional->current_limit(); i++) {
    //   HeapRegion* hr = _optional->region_at(i);
    //   hr->cross_region_ref_update_queue()->organize();
    // }

    G1EvacuationRootClosures* root_cls = pss->closures();
    G1ScanObjsDuringScanRSClosureOptional obj_cl(_g1h, pss);
 ...
 public:
  G1EvacuateOptionalRegionTaskNew(G1CollectedHeap* g1h,
                               G1ParScanThreadStateSet* per_thread_states,
                               G1OptionalCSet* cset,
                               RefToScanQueueSet* queues,
                               uint n_workers) :
    AbstractGangTask("G1 Evacuation Optional Region Task New"),
    _g1h(g1h),
    _per_thread_states(per_thread_states),
    _optional(cset),
    _queues(queues),
  ...
  void work(uint worker_id) {
    ...
    scan_rem_set_source(pss, worker_id);
...
void G1CollectedHeap::evacuate_optional_regions(G1ParScanThreadStateSet* per_thread_states, G1OptionalCSet* ocset) {
  ...
  //mhr: modify
  G1EvacuateOptionalRegionTaskNew task(this, per_thread_states, ocset, _task_queues, workers()->active_workers());
...
void G1CollectedHeap::post_evacuate_collection_set(EvacuationInfo& evacuation_info, G1ParScanThreadStateSet* per_thread_states) {
  ...
  //mhr: modify
  // HeapRegion* hr = NULL;
  // for(size_t i = 0; i < _collection_set._survivor_set_cur_length; i ++){
  //   hr = region_at(_collection_set._original_survivor_regions[i]);
  //   if(hr->is_eden()) {
  //     hr->set_survivor();
  //     _survivor.add(hr);
  //     //_eden
  ...
  public:
    ...
    virtual bool do_heap_region(HeapRegion* r) {
      ...
      if (!r->evacuation_failed()) {
        ...
        //mhr: modify
        //because some regions have already been evacuated by memory server
...
void G1CollectedHeap::free_collection_set(G1CollectionSet* collection_set, EvacuationInfo& evacuation_info, const size_t* surviving_young_words) {
...

  {
    ...
    // for(uint i = 0; i < _collection_set._collection_set_cur_length; i ++){
    //   tty->print("cross queue address: 0x%lx ", (size_t)(region_at(_collection_set._collection_set_regions[i])->cross_region_ref_update_queue()));
    //   region_at(_collection_set._collection_set_regions[i])->cross_region_ref_update_queue()->print_info();
    // }
